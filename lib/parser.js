...
var REGEXES = {
  ...
  forward_in_body : [
    /From\s?:\*?.*(Sent\s?:\*?.*)?(To\s?:\*?.*)?Subject\s?:[\s\w]+/s
  ]
};
...
class Parser {
  ...
  /**
   * Detects whether email is forwarded by email body text
   * @public
   * @param  {string} body
   * @return {boolean} The result
   */
  isForwardedFromBody(body) {
    const _body = this.formatBody(body);

    var _match = loopRegexes(this.__regexes.forward_in_body, _body);

    return _match && _match.length > 1
  }
  ...
  parseBody(body, forwarded = false) {
    ...
    var _match = loopRegexes(this.__regexes.separator_line, _body, Symbol.split);
