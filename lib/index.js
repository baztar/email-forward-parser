...
class EmailForwardParser {
  ...
  read(body, subject = null) {
    ...
    // Check if email was forwarder or not (subject without forward indicator)
    if (!_subject) {
      _forwarded_from_body = this.parser.isForwardedFromBody(body);
    }

    var _forwarded = ((subject && _subject !== null) && true) || _forwarded_from_body !== null || false;
